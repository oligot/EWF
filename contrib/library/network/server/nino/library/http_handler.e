...
feature -- Inherited Features
	...
	execute
		...
		do
			...
			if attached http_server_ip as l_ip then
				create l_factory
				if attached l_factory.create_from_name (l_ip) as l_addr then
					create l_listening_socket.make_server_by_address_and_port (l_addr, l_http_port)
				end
			end
			if not attached l_listening_socket then
...
feature -- Access: configuration
	...
	http_server_ip: detachable STRING
		do
			Result := server_configuration.http_server_ip
		end
